...
function ownSlash(api) {
  return (0, _util.memo)((accountId, era) => api.derive.staking._ownSlash(accountId, era, true));
}

function _ownSlashes(api) {
  return (0, _util.memo)((accountId, eras, withActive) => eras.length ? (0, _rxjs.combineLatest)(eras.map(era => api.derive.staking._ownSlash(accountId, era, withActive))) : (0, _rxjs.of)([]));
}

function ownSlashes(api) {
