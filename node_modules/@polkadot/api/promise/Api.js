...
function promiseTracker(resolve, reject) {
  let isCompleted = false; // eslint-disable-next-line @typescript-eslint/ban-types
...
function decorateMethod(method, options) {
  ...
  return function (...args) {
    ...
    if (!callback) {
      ...
      return method(...actualArgs).pipe((0, _operators.first)()).toPromise();
    }

    return new Promise((resolve, reject) => {
      const tracker = promiseTracker(resolve, reject); // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access,@typescript-eslint/no-unsafe-assignment

      const subscription = method(...actualArgs).pipe( // if we find an error (invalid params, etc), reject the promise
      (0, _operators.catchError)(error => tracker.reject(error)), // upon the first result, resolve with the unsub function
      // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access,@typescript-eslint/no-unsafe-return
      (0, _operators.tap)(() => tracker.resolve(() => subscription.unsubscribe()))).subscribe(result => {
        if (setImmediate) {
...
class ApiPromise extends _base.default {
  ...
  constructor(options) {
    ...
    (0, _classPrivateFieldLooseBase2.default)(this, _isReadyPromise)[_isReadyPromise] = new Promise((resolve, reject) => {
      super.once('ready', () => {
        resolve(this);
      });
      super.once('error', e => {
